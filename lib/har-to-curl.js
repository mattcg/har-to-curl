...
function harToCurl(har, jsonBody) {
	...
	if (har.request) {
		return harToCurl.fromEntry(har, jsonBody);
	}
	...
	if (Array.isArray(har)) {
		return harToCurl.fromEntries(har, jsonBody);
	}
...
harToCurl.fromEntries = function(entries, jsonBody) {
	return entries.map(harToCurl.fromEntry, jsonBody);
};

harToCurl.fromEntry = function(entry, jsonBody) {
	var command;
	...
	if (entry.request.postData) {
        if (jsonBody)
        {
            var jsonParameter = JSON.parse(entry.request.postData.text);
            command += ' -d \'' + JSON.stringify(jsonParameter) + '\'';
        } else {
